  ...
  useEffect(() => {
    if (stage === 1) {
      ...
      setAddresses((prevAddresses) => {
        ...
        if (diff > 0) {
          // Add empty strings to match the new number of friends
          updatedAddresses.push(...Array(diff).fill(""));
        } else if (diff < 0) {
          // Trim the array if numFriends decreases
          updatedAddresses.splice(numFriends + 1);
        }

        // Set the user's location as the default address only if it is not already set
        if (user && user.location && !updatedAddresses.some(addr => addr !== "")) {
          updatedAddresses[0] = user.location;
        }
